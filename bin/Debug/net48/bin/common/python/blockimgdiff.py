...
class Transfer(object):
    ...
    def NetStashChange(self):
        return sum(sr.size() for (_, sr) in self.stash_before) - sum(
            sr.size() for (_, sr) in self.use_stash
        )
...
class BlockImageDiff(object):
    ...
    def __init__(self, tgt, src=None, threads=None, version=4, disable_imgdiff=False):
        if threads is None:
    ...
    def CanUseImgdiff(self, name, tgt_ranges, src_ranges, large_apk=False):
        ...
        if tgt_ranges.extra.get("uses_shared_blocks") or src_ranges.extra.get(
            "uses_shared_blocks"
        ):
        ...
        reason = (
            ImgdiffStats.USED_IMGDIFF_LARGE_APK
            if large_apk
    ...
    def Compute(self, prefix):
        ...
        if common.OPTIONS.cache_size is not None:
            stash_limit = (
                ...
                * common.OPTIONS.stash_threshold
                / self.tgt.blocksize
            )
    ...
    def WriteTransfers(self, prefix):
        ...
        if self.tgt.extended:
            assert (
                WriteSplitTransfers(out, "zero", self.tgt.extended)
        ...
        for xf in self.transfers:
            ...
            if xf.style == "diff":
                ...
                if xf.src_ranges.overlaps(xf.tgt_ranges):
                    if (
                        max_allowed_blocks
                        and stashed_blocks + xf.src_ranges.size() > max_allowed_blocks
                    ):
                        replaced_cmds.append(xf)
                        logger.info(
                            "%10d  %9s  %s", xf.src_ranges.size(), "implicit", xf
                        )
                    else:
                        # The whole source ranges will be stashed for implicit stashes.
                        max_stashed_blocks = max(
                            max_stashed_blocks, stashed_blocks + xf.src_ranges.size()
                        )
    ...
    def ComputePatches(self, prefix):
            ...
            for index, xf in enumerate(self.transfers):
                if xf.style == "zero":
                    ...
                    if xf.src_sha1 == xf.tgt_sha1:
                        ...
                        if xf.src_ranges != xf.tgt_ranges:
                            logger.info(
                                "%10d %10d (%6.2f%%) %7s %s %s (from %s)",
                                tgt_size,
                                tgt_size,
                                100.0,
                                xf.style,
                                xf.tgt_name
                                if xf.tgt_name == xf.src_name
                                else (xf.tgt_name + " (from " + xf.src_name + ")"),
                                str(xf.tgt_ranges),
                                str(xf.src_ranges),
                            )
                    else:
        ...
        with open(prefix + ".patch.dat", "wb") as patch_fd:
            for index, patch_info, _ in patches:
                ...
                tgt_size = xf.tgt_ranges.size() * self.tgt.blocksize
                logger.info(
                    "%10d %10d (%6.2f%%) %7s %s %s %s",
                    xf.patch_len,
                    tgt_size,
                    xf.patch_len * 100.0 / tgt_size,
                    xf.style,
                    xf.tgt_name
                    if xf.tgt_name == xf.src_name
                    else (xf.tgt_name + " (from " + xf.src_name + ")"),
    ...
    def ImproveVertexSequence(self):
        ...
        # executed.
        S = [(u.NetStashChange(), u.order, u) for u in self.transfers if not u.incoming]
        heapq.heapify(S)
    ...
    def ReverseBackwardEdges(self):
        ...
        logger.info(
            "  %d/%d dependencies (%.2f%%) were violated; %d source blocks " "stashed.",
            out_of_order,
            in_order + out_of_order,
            (out_of_order * 100.0 / (in_order + out_of_order))
            if (in_order + out_of_order)
            else 0.0,
            stash_size,
        )
    ...
    def ComputePatchesForInputList(self, diff_queue, compress_target):
                ...
                if compress_target:
                    ...
                    try:
                        ...
                        compress_obj = zlib.compressobj(
                            6, zlib.DEFLATED, -zlib.MAX_WBITS
                        )
                        compressed_data = (
                            compress_obj.compress("".join(tgt_data))
                            + compress_obj.flush()
                        )
        ...
        threads = [threading.Thread(target=diff_worker) for _ in range(self.threads)]
        for th in threads:
            th.start()
        while threads:
            threads.pop().join()
    ...
    def SelectAndConvertDiffTransfersToNew(self, violated_stash_blocks):
        ...
        """
        TransferSizeScore = namedtuple(
            "TransferSizeScore", "xf, used_stash_blocks, score"
        )
        ...
        for xf in self.transfers:
            if xf.style == "diff" and xf.src_sha1 != xf.tgt_sha1:
                use_imgdiff = self.CanUseImgdiff(
                    xf.tgt_name, xf.tgt_ranges, xf.src_ranges
                )
        ...
        for xf_index, patch_info, compressed_size in result:
            ...
            logger.info(
                "%s, target size: %d blocks, style: %s, patch size: %d,"
                " compression_size: %d, ratio %.2f%%",
                xf.tgt_name,
                xf.tgt_ranges.size(),
                diff_style,
                len(xf.patch_info.content),
                compressed_size,
                size_ratio,
            )
            ...
            if len(xf.patch_info.content) >= compressed_size:
                ...
                conversion_candidates.append(
                    TransferSizeScore(xf, used_stash_blocks, -1)
                )
                ...
                score = (
                    (compressed_size - len(xf.patch_info.content))
                    * 100.0
                    / used_stash_blocks
                )
                conversion_candidates.append(
    ...
    def FindTransfers(self):
        ...
        def AddSplitTransfersWithFixedSizeChunks(
            ...
            pieces = 0
            while (
                tgt_ranges.size() > max_blocks_per_transfer
                and src_ranges.size() > max_blocks_per_transfer
            ):
                ...
                Transfer(
                    ...
                    tgt_first,
                    src_first,
                    self.tgt.RangeSha1(tgt_first),
                    self.src.RangeSha1(src_first),
                    style,
                    by_id,
                )
        ...
        def AddSplitTransfers(tgt_name, src_name, tgt_ranges, src_ranges, style, by_id):
            ...
            if self.FileTypeSupportedByImgdiff(tgt_name) and self.tgt.RangeSha1(
                tgt_ranges
            ) != self.src.RangeSha1(src_ranges):
            ...
            AddSplitTransfersWithFixedSizeChunks(
                tgt_name, src_name, tgt_ranges, src_ranges, style, by_id
            )

        def AddTransfer(
            tgt_name, src_name, tgt_ranges, src_ranges, style, by_id, split=False
        ):
            ...
            # much.
            if (
                tgt_name.split(".")[-1].lower() == "odex"
                and tgt_ranges.size() == src_ranges.size()
            ):
                ...
                for src_block, tgt_block in zip(
                    src_ranges.next_item(), tgt_ranges.next_item()
                ):
                ...
                if tgt_changed < tgt_size * crop_threshold:
                    ...
                    logger.info(
                        ...
                        tgt_skipped.size(),
                        tgt_size,
                        tgt_skipped.size() * 100.0 / tgt_size,
                        tgt_name,
                    )
                    AddSplitTransfers(
                        "%s-skipped" % (tgt_name,),
        ...
        def ParseAndValidateSplitInfo(patch_size, tgt_ranges, src_ranges, split_info):
            ...
            for line in split_info[2:]:
                ...
                split_info_list.append(
                    (patch_start, patch_length, split_tgt_ranges, split_src_ranges)
                )
        ...
        def SplitLargeApks():
            ...
            while True:
                ...
                split_info_list = ParseAndValidateSplitInfo(
                    patch_size_total, tgt_ranges, src_ranges, lines
                )
                for index, (
                    patch_start,
                    patch_length,
        ...
        for tgt_fn, tgt_ranges in sorted(self.tgt.file_map.items()):
            if tgt_fn == "__ZERO":
                ...
                AddTransfer(
                    tgt_fn, "__ZERO", tgt_ranges, src_ranges, "zero", self.transfers
                )
                continue
        ...
        threads = [threading.Thread(target=SplitLargeApks) for _ in range(self.threads)]
        for th in threads:
            th.start()
        while threads:
            threads.pop().join()
        ...
        for tgt_name, src_name, tgt_ranges, src_ranges, patch in split_large_apks:
