...
from img2sdat import images, sparse_img
...
def RunAndCheckOutput(args, verbose=None, **kwargs):
    ...
    if proc.returncode != 0:
        raise ExternalError(
            "Failed to run command '{}' (exit code {}):\n{}".format(args, proc.returncode, output)
        )
    return output
...
class BuildInfo(object):
    ...
    def __init__(self, info_dict, oem_dicts=None, use_legacy_id=False):
        ...
        self._device = info_dict.get(
            "ota_override_device", self.GetOemProperty("ro.product.device")
        )
    ...
    def is_vabc(self):
        ...
        vabc_enabled = (
            vendor_prop and vendor_prop.GetProp("ro.virtual_ab.compression.enabled") == "true"
        )
        return vabc_enabled
    ...
    def is_vabc_xor(self):
        vendor_prop = self.info_dict.get("vendor.build.prop")
        vabc_xor_enabled = (
            vendor_prop and vendor_prop.GetProp("ro.virtual_ab.compression.xor.enabled") == "true"
        )
    ...
    def vendor_suppressed_vabc(self):
        vendor_prop = self.info_dict.get("vendor.build.prop")
        vabc_suppressed = vendor_prop and vendor_prop.GetProp("ro.vendor.build.dont_use_vabc")
    ...
    def _GetRawBuildProp(self, prop, partition):
        prop_file = "{}.build.prop".format(partition) if partition else "build.prop"
    ...
    def GetPartitionBuildProp(self, prop, partition):
        ...
        prop_partition = (
            "bootimage" if partition == "boot" or partition == "init_boot" else partition
        )
    ...
    def _ResolveRoProductBuildProp(self, prop):
        ...
        source_order_val = self._GetRawBuildProp("ro.product.property_source_order", None)
        if source_order_val:
    ...
    def _GetBuildId(self):
        ...
        if not vbmeta_digest:
            raise ExternalError("Vbmeta digest isn't provided when using legacy build" " id")
    ...
    def _GetPartitionPlatformVersion(self, partition):
        try:
            return self.GetPartitionBuildProp("ro.build.version.release_or_codename", partition)
        except ExternalError:
            return self.GetPartitionBuildProp("ro.build.version.release", partition)
    ...
    def WriteDeviceAssertions(self, script, oem_no_mount):
        ...
        if not self.oem_dicts:
            raise ExternalError("No OEM file provided to answer expected assertions")
        ...
        for prop in self.oem_props.split():
            ...
            if not values:
                raise ExternalError("The OEM file is missing the property %s" % (prop,))
...
def LoadInfoDict(input_file, repacking=False):
    ...
    """
    if repacking:
        assert isinstance(input_file, str), "input_file must be a path str when doing repacking"
    ...
    if repacking:
        ...
        d["root_fs_config"] = os.path.join(input_file, "META", "root_filesystem_config.txt")
        ...
        for part_name in [
            ...
            if os.path.exists(base_fs_file):
                ...
                logger.warning("Failed to find %s base fs file: %s", part_name, base_fs_file)
                del d[key_name]
...
class PartitionBuildProps(object):
    ...
    def _ReadPartitionPropFile(input_file, name):
        ...
        data = ""
        for prop_file in [
            "{}/etc/build.prop".format(name.upper()),
    ...
    def _LoadBuildProp(self, data):
        for line in data.split("\n"):
            ...
            if line.startswith("import"):
                ...
                if duplicates:
                    raise ValueError(
                        "prop {} is overridden multiple times".format(",".join(duplicates))
                    )
                ...
                if name in self.prop_overrides:
                    raise ValueError(
                        "prop {} is set again after overridden by import " "statement".format(name)
                    )
    ...
    def _ImportParser(self, line):
        ...
        import_path = import_path.replace("/{}".format(self.partition), self.partition.upper())
        ...
        return {key: val for key, val in d.items() if key in self.props_allow_override}

    def GetProp(self, prop):
        return self.build_props.get(prop)


def LoadRecoveryFSTab(read_helper, fstab_version, recovery_fstab_path, system_root_image=False):
...
def MergeDynamicPartitionInfoDicts(framework_dict, vendor_dict):
    ...
    if (framework_dict.get("use_dynamic_partitions") != "true") or (
    ...
    for partition_group in merged_dict["super_partition_groups"].split(" "):
        ...
        merged_dict[key] = uniq_concat(framework_dict.get(key, ""), vendor_dict.get(key, ""))
...
def RunHostInitVerifier(product_out, partition_map):
    ...
    for partition, path in partition_map.items():
        if partition not in allowed_partitions:
            raise ExternalError("Unable to call host_init_verifier for partition %s" % partition)
        ...
        property_contexts_path = os.path.join(
            product_out, path, "etc", "selinux", property_contexts
        )
...
def GetAvbChainedPartitionArg(partition, info_dict, key=None):
    ...
    rollback_index_location = info_dict["avb_" + partition + "_rollback_index_location"]
    return "{}:{}:{}".format(partition, rollback_index_location, pubkey_path)


def _HasGkiCertificationArgs():
    return (
        "gki_signing_key_path" in OPTIONS.info_dict and "gki_signing_algorithm" in OPTIONS.info_dict
    )
...
def BuildVBMeta(image_path, partitions, name, needed_partitions):
    ...
    custom_partitions = (
        OPTIONS.info_dict.get("avb_custom_images_partition_list", "").strip().split()
    )
    ...
    for partition, path in partitions.items():
        if partition not in needed_partitions:
            continue
        assert (
            partition in AVB_PARTITIONS
            or partition in AVB_VBMETA_PARTITIONS
            or partition in custom_partitions
        ), "Unknown partition: {}".format(partition)
        assert os.path.exists(path), "Failed to find {} for {}".format(path, partition)
...
def _MakeRamdisk(sourcedir, fs_config_file=None, ramdisk_format=RamdiskFormat.GZ):
    ...
    if ramdisk_format == RamdiskFormat.LZ4:
        ...
        p2 = Run(
            ["xz", "-f", "-c", "--check=crc32", "--lzma2=dict=32MiB"],
            stdin=p1.stdout,
            stdout=ramdisk_img.file.fileno(),
        )
...
def _BuildBootableImage(
    ...
    if has_ramdisk:
        ...
        ramdisk_img = _MakeRamdisk(sourcedir, fs_config_file, ramdisk_format=ramdisk_format)
    ...
    if _HasGkiCertificationArgs():
        ...
        boot_signature_bytes += _GenerateGkiCertificate(kernel_path, "generic_kernel")

        BOOT_SIGNATURE_SIZE = 16 * 1024
        if len(boot_signature_bytes) > BOOT_SIGNATURE_SIZE:
            raise ValueError(f"GKI boot_signature size must be <= {BOOT_SIGNATURE_SIZE}")
        boot_signature_bytes += b"\0" * (BOOT_SIGNATURE_SIZE - len(boot_signature_bytes))
        assert len(boot_signature_bytes) == BOOT_SIGNATURE_SIZE

        with open(img.name, "ab") as f:
            f.write(boot_signature_bytes)

    if info_dict.get("boot_signer") == "true" and info_dict.get("verity_key"):
    ...
    # Sign the image if vboot is non-empty.
        ...
        cmd = [
            info_dict["vboot_signer_cmd"],
            futility,
            img_unsigned.name,
            info_dict["vboot_key"] + ".vbpubk",
...
def _SignBootableImage(image_path, prebuilt_name, partition_name, info_dict=None):
...
def GetBootableImage(
    name, prebuilt_name, unpack_dir, tree_subdir, info_dict=None, two_step_image=False
):
        ...
        for ramdisk_fragment in ramdisk_fragments:
            ...
            if os.access(fn, os.F_OK):
                ...
                ramdisk_fragment_img = _MakeRamdisk(
                    ramdisk_fragment_root, ramdisk_format=ramdisk_format
                )
...
def GetVendorBootImage(name, prebuilt_name, unpack_dir, tree_subdir, info_dict=None):
    ...
    data = _BuildVendorBootImage(os.path.join(unpack_dir, tree_subdir), "vendor_boot", info_dict)
    if data:
        return File(name, data)
    return None


def GetVendorKernelBootImage(name, prebuilt_name, unpack_dir, tree_subdir, info_dict=None):
...
def UnzipToDir(filename, dirname, patterns=None):
    ...
    if patterns is not None:
        ...
        filtered = [pattern for pattern in patterns if fnmatch.filter(names, pattern)]
...
def GetUserImage(
    ...
    if is_sparse:
        img = GetSparseImage(which, tmpdir, input_zip, allow_shared_blocks, hashtree_info_generator)
        if reset_file_map:
            img.ResetFileMap()
        return img
...
def GetSparseImage(which, tmpdir, input_zip, allow_shared_blocks, hashtree_info_generator=None):
...
def GetMinSdkVersion(apk_name):
    ...
    """
    proc = Run(
        [OPTIONS.aapt2_path, "dump", "badging", apk_name],
...
def SignFile(
    ...
    java_library_path = os.path.join(OPTIONS.search_path, OPTIONS.signapk_shared_library_path)
    ...
    if min_sdk_version is None:
        if not whole_file:
            min_sdk_version = GetMinSdkVersionInt(input_name, codename_to_api_level_map)
...
def CheckSize(data, target, info_dict):
    ...
    if info_dict.get("avb_enable") == "true" and target in AVB_PARTITIONS:
        if size != limit:
            raise ExternalError(
                "Mismatching image size for %s: expected %d actual %d" % (target, limit, size)
            )
    else:
...
def ReadApkCerts(tf_zip):
    ...
    for line in tf_zip.read("META/apkcerts.txt").decode().split("\n"):
        ...
        if cert in SPECIAL_CERT_STRINGS and not privkey:
            ...
            cert.endswith(OPTIONS.public_key_suffix)
            and privkey.endswith(OPTIONS.private_key_suffix)
            and cert[:-public_key_suffix_len] == privkey[:-private_key_suffix_len]
        ):
...
def ParseOptions(argv, docstring, extra_opts="", extra_long_opts=(), extra_option_handler=None):
    ...
    if OPTIONS.search_path:
        os.environ["PATH"] = (
            os.path.join(OPTIONS.search_path, "bin") + os.pathsep + os.environ["PATH"]
        )
    ...
    def PromptResult(self, current):  # pylint: disable=no-self-use
        ...
        for k, v in sorted(current.items()):
            if v:
                ...
                while True:
                    result[k] = getpass.getpass("Enter password for %s key> " % k).strip()
                    if result[k]:
                        break
        return result
...
def ZipWrite(zip_file, filename, arcname=None, perms=0o644, compress_type=None):
    # http://b/18015246
...
def ZipWriteStr(zip_file, zinfo_or_arcname, data, perms=None, compress_type=None):
...
def ComputeDifferences(diffs):
        ...
        try:
            ...
            for d in diff_iter:
                ...
                if patch is None:
                    ...
                    logger.info(
                        "%8.2f sec %8d / %8d bytes (%6.2f%%) %s",
                        dur,
                        len(patch),
                        tf.size,
                        100.0 * len(patch) / tf.size,
                        name,
                    )
            lock.release()
        except Exception:
    ...
    threads = [threading.Thread(target=worker) for i in range(OPTIONS.worker_threads)]
    for th in threads:
        th.start()
    while threads:
        threads.pop().join()
...
class BlockDifference(object):
    def __init__(
        ...
        if version is None:
            version = max(
                int(i) for i in OPTIONS.info_dict.get("blockimgdiff_versions", "1").split(",")
            )
        assert version >= 3
        ...
        if is_dynamic:
            ...
            if OPTIONS.source_info_dict is None:
                _, device_expr = GetTypeAndDeviceExpr("/" + partition, OPTIONS.info_dict)
            else:
                _, device_expr = GetTypeAndDeviceExpr("/" + partition, OPTIONS.source_info_dict)
    ...
    def WriteScript(self, script, output_zip, progress=None, write_verify_script=False):
        if not self.src:
...
def GetTypeAndDevice(mount_point, info, check_no_slot=True):
    ...
    if fstab:
        if check_no_slot:
            assert not fstab[mount_point].slotselect, "Use GetTypeAndDeviceExpr instead"
...
def ExtractPublicKey(cert):
    ...
    assert proc.returncode == 0, "Failed to dump public key from certificate: %s\n%s" % (
        cert,
        stderrdata,
    )
    return pubkey
...
def ExtractAvbPublicKey(avbtool, key):
    ...
    RunAndCheckOutput([avbtool, "extract_public_key", "--key", key, "--output", output])
    return output


def MakeRecoveryPatch(input_dir, output_sink, recovery_img, boot_img, info_dict=None):
    ...
    try:
        ...
        boot_type, boot_device = GetTypeAndDevice("/boot", info_dict, check_no_slot=False)
        recovery_type, recovery_device = GetTypeAndDevice(
            "/recovery", info_dict, check_no_slot=False
        )
    except KeyError:
        return
...
class DynamicPartitionUpdate(object):
    def __init__(self, src_group=None, tgt_group=None, progress=None, block_difference=None):
...
class DynamicPartitionsDifference(object):
    def __init__(
        ...
        block_diff_dict = collections.OrderedDict([(e.partition, e) for e in block_diffs])

        assert len(block_diff_dict) == len(
            block_diffs
        ), "Duplicated BlockDifference object for {}".format(
            [
                partition
                for partition, count in collections.Counter(
                    e.partition for e in block_diffs
                ).items()
                if count > 1
            ]
        )
        ...
        tgt_groups = shlex.split(info_dict.get("super_partition_groups", "").strip())
        src_groups = shlex.split(source_info_dict.get("super_partition_groups", "").strip())

        for g in tgt_groups:
            for p in shlex.split(info_dict.get("super_%s_partition_list" % g, "").strip()):
        ...
        for g in src_groups:
            for p in shlex.split(source_info_dict.get("super_%s_partition_list" % g, "").strip()):
                assert p in self._partition_updates, (
        ...
        target_dynamic_partitions = set(
            shlex.split(info_dict.get("dynamic_partition_list", "").strip())
        )
        block_diffs_with_target = set(p for p, u in self._partition_updates.items() if u.tgt_size)
        assert (
            block_diffs_with_target == target_dynamic_partitions
        ), "Target Dynamic partitions: {}, BlockDifference with target: {}".format(
            list(target_dynamic_partitions), list(block_diffs_with_target)
        )

        source_dynamic_partitions = set(
            shlex.split(source_info_dict.get("dynamic_partition_list", "").strip())
        )
        block_diffs_with_source = set(p for p, u in self._partition_updates.items() if u.src_size)
        assert (
            block_diffs_with_source == source_dynamic_partitions
        ...
        if self._partition_updates:
            logger.info("Updating dynamic partitions %s", self._partition_updates.keys())

        self._group_updates = collections.OrderedDict()

        for g in tgt_groups:
            self._group_updates[g] = DynamicGroupUpdate()
            self._group_updates[g].tgt_size = int(
                info_dict.get("super_%s_group_size" % g, "0").strip()
            )

        for g in src_groups:
            if g not in self._group_updates:
                self._group_updates[g] = DynamicGroupUpdate()
            self._group_updates[g].src_size = int(
                source_info_dict.get("super_%s_group_size" % g, "0").strip()
            )
    ...
    def WriteScript(self, script, output_zip, write_verify_script=False):
        ...
        for p, u in self._partition_updates.items():
            if u.src_size and u.tgt_size and u.src_size > u.tgt_size:
                ...
                u.block_difference.WriteScript(
                    script, output_zip, progress=u.progress, write_verify_script=False
                )
        ...
        script.AppendExtra(
            "assert(update_dynamic_partitions("
    ...
    def _Compute(self):
        ...
        for p, u in self._partition_updates.items():
            if u.src_size and u.tgt_size and u.src_size > u.tgt_size:
                comment("Shrink partition %s from %d to %d" % (p, u.src_size, u.tgt_size))
                append("resize %s %s" % (p, u.tgt_size))

        for g, u in self._group_updates.items():
            if u.src_size is not None and u.tgt_size is None:
                append("remove_group %s" % g)
            if u.src_size is not None and u.tgt_size is not None and u.src_size > u.tgt_size:
        ...
        for p, u in self._partition_updates.items():
            if u.src_group and u.tgt_group and u.src_group != u.tgt_group:
                comment("Move partition %s from default to %s" % (p, u.tgt_group))
...
def GetCareMap(which, imgname):
    ...
    if "extfs_sparse_flag" in OPTIONS.info_dict and not disable_sparse:
        ...
        care_map_ranges = simg.care_map.intersect(RangeSet("0-{}".format(image_blocks)))

    # Otherwise for non-sparse images, we read all the blocks in the filesystem
    # image.
    else:
        care_map_ranges = RangeSet("0-{}".format(image_blocks))
...
def AddCareMapForAbOta(output_file, ab_partitions, image_paths):
    ...
    for partition in ab_partitions:
        ...
        if (
            verity_block_device in OPTIONS.info_dict
            or OPTIONS.info_dict.get(avb_hashtree_enable) == "true"
        ):
            if partition not in image_paths:
                logger.warning(
                    "Potential partition with care_map missing from images: %s",
                    partition,
                )
                continue
            ...
            present_props = [
                x for x in prop_name_list if partition_props and partition_props.GetProp(x)
            ]
    ...
    temp_care_map_text = MakeTempFile(prefix="caremap_text-", suffix=".txt")
