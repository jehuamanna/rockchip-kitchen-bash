...
class Extractor(object):
    ...
    def __ext4extractor(self):
        ...
        import string
        import struct
        fs_config_file = self.FileName.replace("_a", "").replace("_b", "") + '_fs_config'
        fuking_symbols = '\\^$.|?*+(){}[]'
        contexts = self.CONFING_DIR + os.sep + \
            self.FileName.replace("_a", "").replace("_b", "") + "_file_contexts"  # 08.05.18
                ...
                for i in list(entry_inode.xattrs()):
                    if i[0] == 'security.selinux':
                        ...
                        if raw_cap[1] > 65535:
                            cap = '' + \
                                str(hex(int('%04x%04x' %
                                    (raw_cap[3], raw_cap[1]), 16)))
                        else:
                            cap = '' + \
                                str(hex(int('%04x%04x%04x' %
                                    (raw_cap[3], raw_cap[2], raw_cap[1]), 16)))
                        cap = ' capabilities={cap}'.format(cap=cap)
                if entry_inode.is_dir:
                    dir_target = self.EXTRACT_DIR + \
                    ...
                    if cap == '' and con == '':
                        ...
                        if (tmppath).find(' ', 1, len(tmppath)) > 0:
                            spaces_file = self.BASE_MYDIR + 'config' + os.sep + self.FileName.replace("_a", "").replace("_b", "") + '_space.txt'
                            ...
                            self.fsconfig.append(
                                '%s %s %s %s' % (tmppath, uid, gid, mode))
                        else:
                            self.fsconfig.append('%s %s %s %s' % (
                                self.DIR + entry_inode_path, uid, gid, mode))
                    else:
                        if cap == '':
                            tmppath = self.DIR + entry_inode_path
                            if (tmppath).find(' ', 1, len(tmppath)) > 0:
                            ...
                            if con == '':
                                ...
                                if (tmppath).find(' ', 1, len(tmppath)) > 0:
                                    ...
                                    tmppath = tmppath.replace(' ', '_')
                                    self.fsconfig.append('%s %s %s %s' % (
                                        tmppath, uid, gid, mode + cap))
                                else:
                                    self.fsconfig.append('%s %s %s %s' % (
                                        self.DIR + entry_inode_path, uid, gid, mode + cap))
                            else:
                                tmppath = self.DIR + entry_inode_path
                                if (tmppath).find(' ', 1, len(tmppath)) > 0:
                                    spaces_file = self.BASE_MYDIR + 'config' + os.sep + self.FileName.replace("_a", "").replace("_b", "") + '_space.txt'
                    ...
                    try:
                        ...
                        if cap == '' and con == '':
                            ...
                            if (tmppath).find(' ', 1, len(tmppath)) > 0:
                                ...
                                self.fsconfig.append('%s %s %s %s %s' % (
                                    tmppath, uid, gid, mode, link_target))
                            else:
                                self.fsconfig.append('%s %s %s %s %s' % (
                            ...
                            if cap == '':
                                ...
                                if (tmppath).find(' ', 1, len(tmppath)) > 0:
                                    ...
                                    self.fsconfig.append('%s %s %s %s %s' % (
                                        self.DIR + entry_inode_path, uid, gid, mode, link_target))
                                for fuk_symb in fuking_symbols:
                                    tmppath = tmppath = tmppath.replace(
                                        fuk_symb, '\\'+fuk_symb)
                                self.context.append('/%s %s' % (tmppath, con))
                            else:
                                if con == '':
                                    ...
                                    if (tmppath).find(' ', 1, len(tmppath)) > 0:
                                        ...
                                        tmppath = tmppath.replace(' ', '_')
                                        self.fsconfig.append('%s %s %s %s %s' % (
                                            tmppath, uid, gid, mode + cap, link_target))
                                    else:
                                        self.fsconfig.append('%s %s %s %s %s' % (
                        ...
                        if os.name == 'nt':
                            with open(target.replace('/', os.sep), 'wb') as out:
                                ...
                                for index in list(link_target):
                                    tmp = tmp + \
                                        struct.pack(
                                            '>sx', index.encode('utf-8'))
                                out.write(tmp + struct.pack('xx'))
                                os.system('attrib +s "%s"' %
                        ...
                        if entry_inode_path[1:] == entry_name or link_target[1:] == entry_name:
                            self.symlinks.append('%s %s' % (
                                link_target, entry_inode_path[1:]))
                        else:
                            self.symlinks.append('%s %s' % (
                                link_target, self.DIR + entry_inode_path))
                    except:
                        try:
        ...
        if not os.path.isdir(dir_my):
            os.makedirs(dir_my)
        f = open(dir_my + self.FileName.replace("_a", "").replace("_b", "") + '_size', 'tw', encoding='utf-8')
        self.__appendf(os.path.getsize(self.OUTPUT_IMAGE_FILE), dir_my + self.FileName.replace("_a", "").replace("_b", "") + '_size')
        f.close()
        with open(self.OUTPUT_IMAGE_FILE, 'rb') as file:
            ...
            dirr = self.__file_name(os.path.basename(self.EXTRACT_DIR))
            ...
            self.__appendf('\n'.join(self.fsconfig),
                           self.CONFING_DIR + os.sep + fs_config_file)
                ...
                while total_chunks > 0:
                    ...
                    if chunk_header.type == 0xCAC1:  # CHUNK_TYPE_RAW
                        if header.chunk_header_size > EXT4_CHUNK_HEADER_SIZE:
                            img_file.seek(
                                header.chunk_header_size - EXT4_CHUNK_HEADER_SIZE, 1)
    ...
    def checkSignOffset(self, file):
        ...
        if size <= 52428800:
            ...
            mm = mmap.mmap(file.fileno(), 52428800,
                           access=mmap.ACCESS_READ)  # 52428800=50Mb
    ...
    def main(self, target, output_dir):
        ...
        self.BASE_MYDIR = output_dir.replace("_a", "").replace("_b", "") + os.sep
        self.EXTRACT_DIR = os.path.realpath(os.path.dirname(output_dir)) + os.sep + self.__file_name(os.path.basename(output_dir)).replace("_a", "").replace("_b", "") # output_dir
        self.OUTPUT_IMAGE_FILE = self.BASE_DIR + os.path.basename(target)
        self.OUTPUT_MYIMAGE_FILE = os.path.basename(target)
        self.MYFileName = os.path.basename(
            self.OUTPUT_IMAGE_FILE).replace(".img", "")
        ...
        if sys.argv.__len__() == 3:
            ...
            self.CONFING_DIR = '.config'
        
        try:
            if os.path.isdir(self.EXTRACT_DIR):
                shutil.rmtree(self.EXTRACT_DIR)

            if os.path.isdir(self.CONFING_DIR):
                prefix = self.FileName.replace("_a", "").replace("_b", "")

                for fname in os.listdir(self.CONFING_DIR):
                    if fname.startswith(prefix) and (
                        fname.endswith("_file_contexts")
                        or fname.endswith("_fs_config")
                        or fname.endswith("_size")
                        or fname.endswith("_space.txt")
                    ):
                        try:
                            os.remove(os.path.join(self.CONFING_DIR, fname))
                        except:
                            pass
...
if __name__ == '__main__':
    if sys.argv.__len__() == 3:
        Extractor().main(
            sys.argv[1], (sys.argv[2] + os.sep + os.path.basename(sys.argv[1]).split('.')[0]))
    else:
        if sys.argv.__len__() == 2:
            Extractor().main(sys.argv[1], os.path.basename(sys.argv[1]).split('.')[0].replace("_a", "").replace("_b", ""))
